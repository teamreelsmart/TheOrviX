...
export default async function AdminDashboard() {
  ...
  const latestRaw = await Movie.find().sort({ createdAt: -1 }).limit(10).lean();
  const latest = JSON.parse(JSON.stringify(latestRaw)) as DashboardMovie[];
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="space-y-2">
        {latest.map((movie) => (
          <div key={movie._id} className="flex items-center justify-between rounded bg-panel p-3">
            <div>
